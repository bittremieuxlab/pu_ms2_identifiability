...
class InstrumentSettingsEncoder(nn.Module):
    ...
    def forward(self, num_feats, cat_feats, ce_feats, ce_mask):
        ...
        ce_feats_expanded = ce_feats.unsqueeze(-1)

        # Encode each of the 5 slots independently: Output is [B, 5, 16]
        encoded_slots = self.ce_mlp(ce_feats_expanded)

        # Flatten the 5 slots into one long vector: Output is [B, 80]
        concatenated_ce = encoded_slots.view(ce_feats.size(0), -1)

        # Reduce back down to your desired dimension: Output is [B, 16]
        ce_pooled = self.ce_reducer(concatenated_ce)
