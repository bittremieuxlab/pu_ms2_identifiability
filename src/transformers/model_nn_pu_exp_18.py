...
class InstrumentSettingsEncoder(nn.Module):
    def __init__(self, hidden_fc1=64, output_dim=16, n_resolution_bins: int = 10):
        ...
        self.ce_encoder = FloatEncoder(d_model=16)
    ...
    def forward(self, num_feats, cat_feats, ce_feats, ce_mask):
        ...
        ce_mask_bool = ce_mask.unsqueeze(-1).bool()

        # Replace padded/invalid tokens with a large negative number
        ce_encoded_masked = ce_encoded.masked_fill(~ce_mask_bool, -1e9)

        # Apply max pooling along the sequence dimension (dim=1)
        ce_pooled = ce_encoded_masked.max(dim=1)[0]
