...
class InstrumentSettingsEncoder(nn.Module):
    def __init__(self, hidden_fc1=64, output_dim=64, n_resolution_bins: int = 10):
        super().__init__()
        ...
        self.cat_embeddings = nn.ModuleList([
            nn.Embedding(n_resolution_bins, 4),  # resolution category
        ...
        self.meta_mlp = nn.Sequential(
            nn.Linear(52, hidden_fc1),
            nn.ReLU(),
            nn.LayerNorm(hidden_fc1),
...
class MyModel(SpectrumTransformerEncoder):
    ...
    def __init__(self,  instrument_out=64, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.token_projection = nn.Linear(instrument_out, self.d_model)

    def global_token_hook(self, mz_array, intensity_array,
                          inst_rep=None, *args, **kwargs):
        # Removed: precursor_mz encoding and concatenation
        if inst_rep is None:
            raise ValueError("inst_rep must be provided in the batch.")
        global_token = self.token_projection(inst_rep)
        return global_token
