...
class InstrumentSettingsEncoder(nn.Module):
    def __init__(self, d_model: int, hidden_fc1=256,  n_resolution_bins: int = 10):
        super().__init__()
        ...
        self.cat_embeddings = nn.ModuleList([
            nn.Embedding(n_resolution_bins, 8),  # resolution category
            nn.Embedding(2, 4),                  # polarity
            nn.Embedding(2, 4),                  # ionization
            nn.Embedding(2, 4),                  # mild trapping
            nn.Embedding(2, 4),                  # activation1
        ])

        self.ce_encoder = FloatEncoder(d_model=d_model)
        combined_dim = (d_model * 2) + 24
        # 128 (num) +24 (cat) + 128 (ce) = 280
        self.meta_mlp = nn.Sequential(
            nn.Linear(combined_dim, hidden_fc1),
            nn.ReLU(),
            nn.Linear(hidden_fc1, d_model),
            nn.ReLU()
        )
    ...
    def forward(self, num_feats, cat_feats, ce_feats, ce_mask):
        ...
        ce_encoded = self.ce_encoder(ce_feats)
        ...
        return self.meta_mlp(combined)


class MyModel(SpectrumTransformerEncoder):
    """Our custom model class."""

    def __init__(self, instrument_hidden=256, instrument_out=16, *args, **kwargs):
        ...
        self.token_projection = nn.Linear(self.d_model + self.d_model, self.d_model)

        self.apply(self.init_weights)

    def global_token_hook(self, mz_array, intensity_array, precursor_mz=None,
                          inst_rep=None, *args, **kwargs):
        if precursor_mz is None or inst_rep is None:
        ...
        combined = torch.cat([mz_rep, inst_rep], dim=-1)
...
class SimpleSpectraTransformer(pl.LightningModule):
    def __init__(
        ...
        self.fc_combined = nn.Linear(d_model + d_model, d_model)
    ...
    def forward(self, batch):
        ...
        inst_rep = self.instrument_encoder(num_feats, cat_feats, ce_feats, ce_mask)

        spectra_emb, _ = self.spectrum_encoder(
            mz_array,
            intensity_array,
        ...
        # Extract the global token (which now contains all the metadata!)
        global_emb = spectra_emb[:, 0, :]

        # Pass directly to final output
        combined_emb = torch.cat([global_emb, inst_rep], dim=-1)
    ...
    def configure_optimizers(self):
        ...
        instrument_param_ids = {id(p) for p in instrument_params}
        transformer_params = [
            p for p in self.spectrum_encoder.parameters()
            if id(p) not in instrument_param_ids
        ]

        # 3. Assign the correct learning rates to each non-overlapping group
        param_groups = [
