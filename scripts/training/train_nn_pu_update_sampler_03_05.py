...
from src.transformers.model_nn_pu_v16 import SimpleSpectraTransformer
...
class MZPairedBatchSampler(Sampler):
    """
    Yields batches spanning multiple m/z bins; for each bin included in a batch,
    both labeled (label==1) and unlabeled (label==0) samples are present.
    ...
    def __init__(
        ...
        n_bins_per_batch: int = 5,
        seed: int = 0,
    ):
        self.seed = seed
        self._epoch = 0

        # Linear-spaced bins from 70 to mz_cap
        lin_bins = np.linspace(70, mz_cap, n_bins + 1)

        useful_mask = mz_values <= mz_cap
        useful_indices = np.where(useful_mask)[0]
        mz_useful = mz_values[useful_mask]
        labels_useful = labels[useful_mask]
        bin_idx = np.clip(np.digitize(mz_useful, lin_bins) - 1, 0, n_bins - 1)

        self.bin_labeled = []
        self.bin_unlabeled = []
        for b in range(n_bins):
            in_bin = np.where(bin_idx == b)[0]
            orig_idx = useful_indices[in_bin]
            bin_labels = labels_useful[in_bin]
            lbl = orig_idx[bin_labels == 1.0]
            unl = orig_idx[bin_labels == 0.0]
            if len(lbl) >= 1 and len(unl) >= 1:
                self.bin_labeled.append(lbl)
                self.bin_unlabeled.append(unl)

        n_valid = len(self.bin_labeled)
        if n_valid == 0:
            raise ValueError(
                "MZPairedBatchSampler: no m/z bin contains both labeled and "
                "unlabeled samples. Check dataset or mz_cap setting."
            )

        self.n_bins_per_batch = min(n_bins_per_batch, n_valid)
        self.labeled_per_bin = max(1, round(batch_size * labeled_fraction / self.n_bins_per_batch))
        slots_per_bin = max(2, batch_size // self.n_bins_per_batch)
        self.unlabeled_per_bin = max(1, slots_per_bin - self.labeled_per_bin)
        self.actual_batch_size = self.n_bins_per_batch * (self.labeled_per_bin + self.unlabeled_per_bin)

        total_labeled = sum(len(b) for b in self.bin_labeled)
        self.n_batches = max(1, total_labeled // (self.labeled_per_bin * self.n_bins_per_batch))

        print(
            f"MZPairedBatchSampler: {n_valid} valid bins, "
            f"{self.n_bins_per_batch} bins/batch, "
            f"{self.labeled_per_bin} labeled + {self.unlabeled_per_bin} unlabeled per bin, "
            f"actual batch size={self.actual_batch_size}, "
            f"{self.n_batches:,} batches/epoch, "
            f"dropped {(~useful_mask).sum():,} high-mz samples"
        )
    ...
    def __iter__(self):
        ...
        for _ in range(self.n_batches):
            selected = rng.choice(n_valid, size=self.n_bins_per_batch, replace=False)
            batch = []
            for b in selected:
                lbl_pool = self.bin_labeled[b]
                unl_pool = self.bin_unlabeled[b]
                batch.extend(
                    rng.choice(
                        lbl_pool,
                        size=self.labeled_per_bin,
                        replace=len(lbl_pool) < self.labeled_per_bin,
                    ).tolist()
                )
                batch.extend(
                    rng.choice(
                        unl_pool,
                        size=self.unlabeled_per_bin,
                        replace=len(unl_pool) < self.unlabeled_per_bin,
                    ).tolist()
                )
            rng.shuffle(batch)
...
class LanceBatchCollator:
    ...
    Supports ``instrument_settings`` width 13 (legacy Lance) or 14 (``create_lance_train_res_cat_num_23_04``).
    Width 14 is folded to the legacy 13-slot order before ``batch["instrument_settings"]`` is returned,
    because ``SimpleSpectraTransformer`` reads polarity at index 9 for nnPU priors.
    """
    ...
    def __call__(self, batch_indices: list[int]) -> dict:
        ...
        inst_dim = instrument_settings.shape[-1]

        # Legacy 13: [width, inj, AGC | CE×5 | raw_resolution | 4×metadata]; polarity at index 9 for nnPU.
        # 14 from create_lance_train_res_cat_num_23_04.py: scaled [width, inj, Orbitrap Res, AGC, CE×5],
        #   then raw resolution (9) and four scalars (10–13). Re-pack to 13 so model + splits stay aligned.
        if inst_dim == 14:
            num_feats = instrument_settings[:, [0, 1, 3]]
            ce_feats = instrument_settings[:, 4:9]
            raw_resolution = instrument_settings[:, 9]
            other_cats = instrument_settings[:, 10:14].round()
            instrument_settings = torch.cat(
                [num_feats, ce_feats, raw_resolution.unsqueeze(1), other_cats], dim=1
            )
        elif inst_dim == 13:
            num_feats = instrument_settings[:, 0:3]
            ce_feats = instrument_settings[:, 3:8]
            raw_resolution = instrument_settings[:, 8]
            other_cats = instrument_settings[:, 9:13].round()
        else:
            raise ValueError(
                f"instrument_settings last dim is {inst_dim}; expected 13 (legacy Lance) or 14 "
                f"(create_lance_train_res_cat_num_23_04)."
            )
        ...
        # num_feats, ce_feats, raw_resolution, other_cats are already set above (13- or 14-source).

        # Map each unique resolution value to a discrete category index.

        # 7500, 15000, 17500, 30000, 35000, 60000, 70000, 120000, 140000, 240000
        resolution_values = [
            7500.0,
            15000.0,
            17500.0,
        ...
        res_to_idx = {v: i for i, v in enumerate(resolution_values)}

        raw_res_list = raw_resolution.detach().cpu().tolist()
        resolution_cat_list = [
            res_to_idx.get(float(r), len(resolution_values) - 1) for r in raw_res_list
        ]
        resolution_cat = torch.tensor(
            resolution_cat_list, device=raw_resolution.device, dtype=torch.long
        )  # [B]

        # Final categorical tensor: first column is resolution category, followed by the 4 binary metadata features.
        cat_feats = torch.cat(
            [resolution_cat.unsqueeze(1), other_cats], dim=1
        )  # [B, 5]

        # CE mask for non-zero CEs
        ce_mask = ce_feats > 0                             # [B, 5] boolean mask for non-zero CEs
        ...
        batch_dict = {
            ...
            # structured metadata for encoder (matches BCE model)
            "num_features": num_feats,
            "cat_features": cat_feats,
            "ce_features": ce_feats,
            "ce_mask": ce_mask,
            # raw instrument settings used inside PU step for priors
...
class ImprovedPyTorchSklearnWrapper:
    ...
    def fit(
        ...
        _train_table = (
            lance.dataset(train_lance_path)
            .to_table(columns=["precursor_mz", "label"])
            .to_pydict()
        )
        mz_values = np.array(_train_table["precursor_mz"])
        label_values = np.array(_train_table["label"])

        train_sampler = MZPairedBatchSampler(
            mz_values=mz_values,
            labels=label_values,
            batch_size=self.batch_size,
            n_bins=10,
        ...
        script_logger.info(
            f"Trainer configuration: accelerator={accelerator}, devices={devices}, precision={precision}, strategy={strategy}")
...
def main(args):
    ...
    experiment_name = "training_nn_pu_model_29_04"
