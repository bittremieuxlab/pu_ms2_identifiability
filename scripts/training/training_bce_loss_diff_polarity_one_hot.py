...
class LanceIndexDataset(Dataset):
    ...
    def __init__(self, lance_dataset_path: str, polarity: int = None, rank: str = "0"):
        ...
        script_logger.info(
            f"LanceIndexDataset (rank {rank}) initialized. "
            f"Filtered to {self.length} samples "
            f"(Target Polarity: {'Positive' if polarity == 1 else 'Negative' if polarity == 0 else 'All'})"
        )
...
def main(args):
    ...
    experiment_name = "training_from_lance_02_11_bce_loss_diff_params"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--polarity", type=int, choices=[0, 1], help="0 for Negative, 1 for Positive"
    )
    parser.add_argument(
