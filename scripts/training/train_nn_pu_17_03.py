...
from src.transformers.model_nn_pu_26_03_global_token_v2 import SimpleSpectraTransformer
...
class LanceBatchCollator:
    ...
    def __call__(self, batch_indices: list[int]) -> dict:
        ...
        # Split instrument_settings into numerical, categorical, and CE features.
        # Layout from Lance creation:
        # indices 0-2: numerical (MS2 Isolation Width, Ion Injection Time, AGC Target)
        # indices 3-7: HCD Energy(1-5) (scaled)
        # index 8: Orbitrap Resolution (raw numeric, from a fixed discrete set)
        # indices 9-12: categorical scalars (Polarity, Ionization, Mild Trapping Mode, Activation1)
        num_feats = instrument_settings[:, 0:3]            # [B, 3]
        ce_feats = instrument_settings[:, 3:8]             # [B, 5]
        raw_resolution = instrument_settings[:, 8]         # [B]
        other_cats = instrument_settings[:, 9:13].round()  # [B, 4], values 0/1
...
def main(args):
    ...
    experiment_name = "training_nn_pu_model_17_03"
